...
namespace Proje_filmolog.Other
{
    ...
    public partial class InfoWin : Window
    {
        ...
        public InfoWin() => InitializeComponent();
        ...
        {
            if (!string.IsNullOrEmpty(PassWordTbox.Password) && !string.IsNullOrEmpty(PassWord2Tbox.Password) && !string.IsNullOrEmpty(TelNoTbox.Text.Trim()) && !string.IsNullOrEmpty(RealNameTbox.Text.Trim()))
            {
                SQLiteConnection con = new SQLiteConnection(Adress);
                if (con.State == ConnectionState.Open) con.Close();
                con.Open();
                using (SQLiteCommand cmd = new SQLiteCommand("UPDATE Users SET password=@password,realName=@realName,telNo=@telNo WHERE userName=@userName", con))
                {
                    cmd.Parameters.AddWithValue("@userName", TEMP.active_User);
                ...
                MessageBox.Show("Gerekli Alanlar Boş olduğu İçin Değişiklikler Kaydedilmedi!!", "DİKKAT", MessageBoxButton.OK, MessageBoxImage.Error);
        }
